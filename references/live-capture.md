...
분석용으로 화면을 보는 것과 별개로, **pptx에 넣으려면 반드시 PNG 파일이
...
- **파일 캡처**: `computer` 도구의 screenshot 액션을 `save_to_disk: true`로 호출하면
  ...
  세션 후 사라질 수 있기 때문이다).
- **스크롤이 긴 화면(세로로 긴 화면)**: 매뉴얼 전체에서 캡처는 **동일한 렌더 폭**으로
  들어가야 일관성이 좋다(폭이 일관성의 앵커). 세로로 긴 화면을 풀페이지 한 장으로 담으면
  문서에서 폭이 줄어 다른 캡처와 크기가 달라진다. 두 가지로 대응한다:
  1. **논리 분할 캡처(기본·권장)**: 기능 경계(검색·필터부 / 목록·표부 / 상세·버튼부)로
     나눠 각 조각을 표준 뷰포트로 개별 캡처(SCR-ID 또는 -a/-b) → 각 조각이 정상 화면과
     동일 크기로 균일하고, 배지·설명도 조각별로 1:1 대응한다. 요소(표 행·카드) 절단이 없다.
  2. **풀페이지 + 자동 타일(폴백)**: 논리 분할이 어려우면 `--full-page`로 한 장 캡처한다.
     빌더가 폭을 줄이는 대신 **표준 비율 밴드로 자동 분할**하며, 절단선은 배경 여백 행에
     스냅해 표 행·카드를 자르지 않는다. 세로 비율이 표준(1.12)에서 크게 벗어나면
     cdp_capture 가 안내를 출력한다.
...
- 화면별 캡처(표준 호출 — 화면당 1회 실행하고, 옵션을 습관적으로 함께 쓴다):
  ...
  - **뷰포트 통일(폭 = 일관성 앵커)**: 한 매뉴얼의 모든 캡처는 **동일한 뷰포트 크기**로
    찍어 문서에서 **동일한 렌더 폭**으로 들어가게 한다 — 폭이 제각각이면 완성도가 급락한다.
    `--viewport` 미지정 시 표준값 **1600x1080**(비율 1.48)이 자동 적용되므로 창 크기와
    무관하게 일관된다. 이 표준을 지키면 빌더가 모든 캡처를 전폭(세로형 6.4in)
    으로 균일 렌더한다. 다른 값을 쓰면 config.md에 기록하고 전 화면에 같은 값을 명시한다
    (현재 창 크기를 그대로 쓰려면 `--viewport none` — 비결정적이므로 권장하지 않음).
    세로로 긴 풀페이지 캡처만 예외로, 빌더가 표준 비율 밴드로 자동 분할해 폭을 지킨다
    (위 '스크롤이 긴 화면' 참조).
...
| 목록 표 | 표의 항목(열) 이름, 정렬/페이징 여부, 줄 클릭 동작 |
...
- [ ] 표의 항목(열)이 폭 부족으로 세로쓰기·말줄임되지 않았는가 (있으면 `--viewport` 폭 상향)
